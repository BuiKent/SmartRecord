            ...
            ACTION_STOP -> {
                ...
                serviceScope.launch {
                    try {
                        ...
                        // 2. Stop auto-save job (KHÔNG force save vì stopRecordingAndSave() sẽ tự lưu)
                        // ⚠️ CRITICAL: KHÔNG gọi forceSaveNow() ở đây vì:
                        // - forceSaveNow() có thể tạo recording với title/ID khác → duplicate
                        // - stopRecordingAndSave() sẽ tự lưu recording với title được generate đúng
                        autoSaveManager.stopAutoSave()
                        AppLogger.d(TAG_SERVICE, "Auto-save stopped")
                        
                        // 3. Tạo Recording object để lưu
                        // ⚠️ CRITICAL: KHÔNG gọi audioRecorder.stopRecording() ở đây
                        // stopRecordingAndSave() sẽ tự gọi audioRecorder.stopRecording() bên trong
                        val tempRecording = Recording(
                            id = recordingId,
                            title = "", // Will be auto-generated by stopRecordingAndSave()
                            filePath = filePath, // Dùng filePath từ repository state (chưa stop)
                        ...
                        // ⚠️ CRITICAL: stopRecordingAndSave() sẽ tự gọi audioRecorder.stopRecording() bên trong
                        // Đảm bảo chỉ gọi 1 lần, giống như logic trong RecordViewModel.onStopClick()
